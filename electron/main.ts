...
const logMsg = (msg, type = 'debug') => {
  ...
  msg = `[${appVersion}] ${msg}`;
  consoleWin.webContents.send(MAIN_EVENTS.console_log, {
    msg,
    color: colors[type],
  });
};
...
async function createWindow() {
  ...
  mainWindowBounds.icon = path.join(
    __dirname,
    '/dist/neb-desktop/assets/icons/win/icon.png.png'
  );
...
function createConsoleWindow() {
  ...
  consoleWin.loadURL(
    url.format({
      pathname: path.join(
        __dirname,
        `/dist/neb-desktop/assets/views/console.html`
      ),
      protocol: 'file:',
      slashes: true,
    })
...
async function trackDevices() {
  ...
  if (process.platform !== 'win32') {
    return logMsg(process.platform, 'debug');
  }
...
async function prepareHeadsetOnOfflineMode() {
  ...
  try {
    if (!headsetDevice) {
      ...
      return sendEvToWin(MAIN_EVENTS.offline_headset_ready, {
        ready: false,
        headsetDevice,
        err: 'No Authorized Headset connected!',
      });
    }
    const ipFile = createIpFile();
    const transfer = await client.push(
      headsetDevice.id,
      ipFile,
      '/sdcard/Download/ip.json'
    );
    transfer.once('end', () => {
    ...
    sendEvToWin(MAIN_EVENTS.offline_headset_ready, {
      ...
      err:
        err.message ||
        'ADB Failure: Something went wrong while pushing file to connected headset',
    });

    const msg =
      'Error...' + 'prepareHeadsetOnOfflineMode' + JSON.stringify(err);
    logMsg(msg, 'error');
    sendEvToWin(MAIN_EVENTS.error, err);
  }
}
